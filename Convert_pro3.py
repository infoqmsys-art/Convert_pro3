...
class ConvertPro3App:
    ...
    def do_source_update(self, status_cb=None) -> dict:
        """
        실행 폴더(self.base_dir)에서 git pull --ff-only.
        개발 PC push 후 서버/실행 PC의 [업데이트] 버튼에서 호출.
        """
        if not self.update_manager:
            from utils.update_manager import UpdateManager
            self.update_manager = UpdateManager(self.logger)
        return self.update_manager.pull_latest(str(self.base_dir), status_cb=status_cb)
