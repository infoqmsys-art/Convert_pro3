...
# 응답 헤더 설정 (Content-Type을 application/json으로 설정)
...
def convert_timestamp(seconds_since_2000):
    ...
    # 올림 및 내림 처리
    if kst_date.second >= 50:  # 59분 50초 이상일 경우 올림
        kst_date += timedelta(minutes=1)
        kst_date = kst_date.replace(second=0)
    elif kst_date.second < 10:  # 10초 미만일 경우 내림
        kst_date = kst_date.replace(second=0)
...
try:
    ...
    parsed_data = {
        ...
        "AmountCH0": round(payload[15], 4),
        "AmountCH1": round(payload[16], 4),
        "AmountCH2": round(payload[17], 4),
        "AmountCH3": round(payload[18], 4),
        "AmountCH4": round(payload[19], 4),
        "AmountCH5": round(payload[20], 4),
        "AmountCH6": round(payload[21], 4),
        "AmountCH7": round(payload[22], 4),
        "initDegreeX": round(payload[23], 4),
        "initDegreeY": round(payload[24], 4),
        "initDegreeZ": round(payload[25], 4),
        "initCrack": round(payload[26], 4),
        "initCH1": round(payload[27], 4),
        "initCH2": round(payload[28], 4),
        "initCH3": round(payload[29], 4),
        "initCH4": round(payload[30], 4),
        "initCH5": round(payload[31], 4),
        "initCH6": round(payload[32], 4),
        "initCH7": round(payload[33], 4),
    ...
    csv_dir = os.path.join(base_dir, "csv", device_id)  # CSV 저장 디렉토리 경로
    csv_file_path = os.path.join(csv_dir, f"{line_number}.csv")  # CSV 파일 경로
    ...
    os.makedirs(csv_dir, exist_ok=True)  # CSV 저장 디렉토리 생성
    ...
    # 리퍼런스를 변수로 설정하여 응답을 구분
    reference_device_id = "1200000000"  # -------------------------------------------------------------------------------리퍼런스 값

    if device_id == reference_device_id:
        # 리퍼런스 기기일 경우 응답을 보내지 않음
        print("No response for device with ID:", reference_device_id)
    else:
        # 리퍼런스 기기가 아닌 다른 기기들에게 응답
        response = {
            "status": "success",
            "lineNumber": line_number,
            "data": {"nid": reference_device_id, "sendPeriod": "300", "reset": "1"}
        }
