...
<head>
    ...
    <style>
        ...
        .new-btn {{
            position: absolute;
            top: 5px;
            left: 130px; /* go to viewer 버튼 옆으로 위치 조정 */
            color: white;
            background-color: #808080;
            padding: 10px 20px;
            text-decoration: none;
    ...
    <script>
        ...
        const scrollStep = 100; // Adjust scroll step for smoother movement
        const scrollIntervalTime = 7000; // Adjust for slower scrolling
        ...
        function autoScroll() {{
            ...
            window.scrollBy({{ top: scrollStep * scrollDirection, behavior: 'smooth' }});
        }}
        ...
        window.addEventListener('keydown', resetScrollTimer);
    </script>
</head>
<body>
    ...
    <a href="http://qmview.iptime.org:8080/view/demo.py" class="new-btn">DEMO</a>
...
for site_code, entries in grouped_data.items():
    ...
    for file, data in sorted(entries, key=lambda x: x[1].get("deviceId", "Unknown")[-6:]):
        device_id = data.get("deviceId", "Unknown")
