...
try:
    ...
    for ch in custom_header[1:8]:  # ch1 ~ ch8
        plt.plot(data["time"], data[ch], label=ch)
    interval = 10  # 10개 간격으로 레이블 표시
    x_ticks = [0, len(data["time"]) // 2, len(data["time"]) - 1]  # 처음, 중간, 끝 인덱스
    x_labels = data["time"].iloc[x_ticks]  # 해당 인덱스의 시간 값 선택
    plt.xticks(ticks=x_ticks, labels=x_labels, rotation=0)  # X축 레이블 설정
    plt.xlabel("Time")
    ...
    plt.savefig(graph_path, dpi=150, transparent=True)  # 투명 배경 설정
    plt.close()
