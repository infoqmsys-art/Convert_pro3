...
def blast_to_csv_row(parsed: dict[str, str]) -> dict[str, str] | None:
    """파싱 결과 → CSV 7열 (START_TIME·END_TIME 24h)."""
    end_24 = normalize_nb_time(parsed.get("End_Time"))
    if not end_24:
        return None
    pvs = (parsed.get("PVS_Peak") or "").strip()
    return {
        "START_TIME": format_convert_timestamp(parsed.get("Start_Time")),
        "END_TIME": format_convert_timestamp(parsed.get("End_Time")),
        "VelPeak_X": parsed.get("VelPeak_X", ""),
        "VelPeak_Y": parsed.get("VelPeak_Y", ""),
        "VelPeak_Z": parsed.get("VelPeak_Z", ""),
        "PVS_Peak": pvs,
        "Kine": format_kine_from_pvs(pvs),
    }
...
def sync_neo_blast_folder(
    ...
    Returns: {parsed, appended, skipped_dup, failed, total_rows}
    """
    ...
    parsed = appended = skipped_dup = failed = 0
    ...
    if logger:
        logger.log(
            ...
            f"실패 {failed}, CSV 총 {total_rows}행 → {out_csv_path}",
            level="INFO",
        )
