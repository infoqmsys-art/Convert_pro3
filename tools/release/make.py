...
# 경로 설정 (tools/release/make.py → 프로젝트 루트는 두 단계 위)
if getattr(sys, 'frozen', False):
    SCRIPT_DIR = Path(sys.executable).parent
else:
    SCRIPT_DIR = Path(__file__).resolve().parent

PROJECT_ROOT = SCRIPT_DIR.parent.parent  # tools/release → repo root
VERSION_FILE = PROJECT_ROOT / "version.py"
CHANGELOG_FILE = PROJECT_ROOT / "CHANGELOG.md"
SPEC_FILE = PROJECT_ROOT / "Convert_pro3.spec"
UPDATER_SPEC = SCRIPT_DIR / "updater.spec"
...
def build():
    ...
    result = subprocess.run(
        ['pyinstaller', str(UPDATER_SPEC), '--clean'],
        cwd=PROJECT_ROOT,
        capture_output=False
    )
...
def deploy():
    ...
    with zipfile.ZipFile(zip_file, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        # 실행 경로에 외부로 덮어쓸 수 있는 웹/서버 소스도 포함
        for folder_name in ("monitoring", "measurement_portal", "survey_mgmt_portal"):
            ...
            for p in src_dir.rglob("*"):
                if p.is_file():
                    ...
                    zf.write(p, f"{folder_name}/{rel.as_posix()}")
            ...
            if choice == '0':
                ...
                if bump_version('patch'):
                    if build():
                        ...
                        print_info("소스 push: build_scripts\\push.bat \"커밋 메시지\"")
                        print_info("EXE 배포 순서: tools\\release\\DEPLOY.txt")
